module Example2 {
  ...
  // default 引数の推論型抽出
  let numericString = '0';
  function toNumber(str = numericString) {  // str は optional 型
    return Number(str);
  }
  ...
  const defaultArg = getSomething();
  function doSomething(arg = defaultArg) {  // arg は optional 型
    return {
      foo: arg.foo.toUpperCase(),
      bar: arg.bar + 1,
      baz: !arg.baz
    }
  }
